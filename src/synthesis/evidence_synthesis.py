...
class EvidenceSynthesisEngine:
    ...
    def __init__(self, 
                 ...
                 ai_client: Union[OpenAIClient, XAIClient]):
        """
    ...
    async def build_evidence_table(self, 
                                   included_studies: List[Dict[str, Any]],
                                   research_question: str,
                                   outcomes: List[str]) -> List[EvidenceRow]:
        """
    ...
    async def perform_synthesis(self, 
                               ...
                               synthesis_method: SynthesisMethod,
                               task_id: str,
                               research_question: str) -> SynthesisResult:
        """
    ...
    async def _extract_evidence_row(self, 
                                   study: Dict[str, Any],
                                   research_question: str,
                                   outcome: str) -> Optional[EvidenceRow]:
    ...
    async def _narrative_synthesis(self, 
                                  evidence_rows: List[EvidenceRow],
                                  research_question: str) -> Tuple[List[Dict[str, Any]], str]:
    ...
    async def _thematic_synthesis(self, 
                                 evidence_rows: List[EvidenceRow],
                                 research_question: str) -> Tuple[List[Dict[str, Any]], str]:
    ...
    async def _meta_aggregation_synthesis(self, 
                                        evidence_rows: List[EvidenceRow],
                                        research_question: str) -> Tuple[List[Dict[str, Any]], str]:
    ...
    async def _analyze_contradictions(self, 
                                    outcome: str,
                                    evidence_rows: List[EvidenceRow]) -> Dict[str, Any]:
    ...
    async def _assess_outcome_confidence(self, 
                                       outcome: str,
                                       evidence_rows: List[EvidenceRow]) -> Dict[str, Any]:
    ...
    async def _assess_overall_confidence(self, 
                                       outcome_confidences: Dict[str, Any],
                                       themes: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _identify_limitations(self, 
        ...
        if len(contradictions) > 0:
            limitations.append(f"Contradictory findings identified across studies (n={len(contradictions)}) suggest caution in interpretation.")
        
        # Publication bias
        limitations.append("Potential publication bias cannot be ruled out without comprehensive grey literature search.")
...
---"""
    ...
    def _format_evidence_for_confidence(self, evidence_rows: List[EvidenceRow]) -> str:
        ...
        formatted_evidence = []
        for row in evidence_rows:
            evidence_text = f"Study {row.study_id}: {row.study_design}, " \
...
async def example_evidence_synthesis():
    ...
    print(f"Overall confidence: {synthesis_result.confidence_assessment.get('overall', {}).get('overall_confidence', 'unknown')}")

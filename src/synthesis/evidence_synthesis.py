...
class EvidenceSynthesisEngine:
        ...
        for outcome, rows in outcomes_evidence.items():
            ...
            if contradiction_analysis.get("has_contradictions", False):
                contradictions.append(
                    {
                        "outcome": outcome,
                        "conflicting_studies": contradiction_analysis.get(
                            "conflicting_studies", []
                        ),
                        "contradiction_type": contradiction_analysis.get(
                            "type", "unknown"
                        ),
                        "description": contradiction_analysis.get("description", ""),
                        "severity": contradiction_analysis.get("severity", "moderate"),
                        "potential_explanations": contradiction_analysis.get(
                            "explanations", []
                        ),
                    }
                )
    ...
    async def assess_confidence(
        ...
        overall_confidence = await self._assess_overall_confidence(
            confidence_assessment, themes
        )
    ...
    async def _narrative_synthesis(
        ...
        try:
            ...
            return synthesis_data.get("themes", []), synthesis_data.get(
                "narrative_summary", ""
            )
    ...
    async def _assess_outcome_confidence(
        self, outcome: str, evidence_rows: List[EvidenceRow]
    ) -> Dict[str, Any]:
    ...
    async def _identify_limitations(
        ...
        # Study quality
        quality_scores = [
            row.quality_score for row in evidence_rows if row.quality_score
        ]
        if not quality_scores:
            limitations.append(
                "Quality assessment scores not available for all studies."
            )
...
---"""
    ...
    async def _store_synthesis_result(self, synthesis_result: SynthesisResult) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Stored synthesis result: {synthesis_result.synthesis_id}"
            )
...
async def example_evidence_synthesis(
    research_question: Optional[str] = None, example_domain: str = "research"
):

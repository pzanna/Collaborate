...
class EnhancedScreeningWorkflow:
    ...
    async def title_abstract_screening(
        self, studies: List[Dict[str, Any]], criteria: Dict[str, Any], task_id: str
    ) -> Dict[str, Any]:
        """
    ...
    async def _screen_title_abstract_study(
        ...
        try:
            response = self.ai_client.get_response(
                user_message=prompt, system_prompt=self._get_screening_system_prompt()
            )
            ...
            human_review_required = (
                decision_data["confidence"] < self.confidence_threshold
                or decision_data["decision"] == "uncertain"
                or self.require_human_review
                and decision_data["decision"] == "exclude"
            )
            ...
            return ScreeningResult(
                ...
                exclusion_reason=(
                    ExclusionReason(decision_data["exclusion_reason"])

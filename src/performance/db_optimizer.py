...
class QueryOptimizer:
        ...
        try:
            with self.connection_pool.get_connection() as conn:
                ...
                metrics.average_query_time_ms = (
                    (metrics.average_query_time_ms + execution_time_ms) / 2
                )
    ...
    def get_query_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'recent_queries': len([q for q in self.query_stats if (datetime.now(timezone.utc) - q.timestamp).seconds < 300])
        }
    ...
    async def analyze_index_usage(self) -> Dict[str, Any]:
        ...
        try:
            with self.connection_pool.get_connection() as conn:
                ...
                if len(indexes) == 0:
                    analysis['recommendations'].append("No indexes found - consider adding indexes on frequently queried columns")
                elif len(indexes) > 20:
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            'connection_pool': {
                ...
                ) if self.connection_pool.metrics.connection_pool_size > 0 else 0
            },
...
async def demo_database_optimizer():
    ...
    for i in range(100):
        await optimizer.execute_optimized_query(
            ...
            (f"Study {i}", f"Author {i % 10}", 2020 + (i % 5), f"Abstract for study {i}", "completed" if i % 3 == 0 else "pending")
        )

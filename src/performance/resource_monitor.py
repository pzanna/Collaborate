...
class SystemResourceMonitor:
    ...
    def get_metrics_summary(self, duration_minutes: int = 10) -> Dict[str, Any]:
        ...
        return {
            ...
            'alerts_triggered': len([m for m in history if m.cpu_percent > self.cpu_threshold or m.memory_percent > self.memory_threshold])
        }
...
class PerformanceProfiler:
    ...
    def get_slowest_functions(self, limit: int = 10) -> List[PerformanceProfile]:
        ...
        return sorted(
            ...
            key=lambda p: p.average_time_ms,
            reverse=True
        )[:limit]
...
async def demo_resource_monitor():
    ...
    result = await monitor.monitor_operation(
        "complex_operation",
        lambda: time.sleep(0.1) or "Complex task done"
    )

...
try:
    from ..ai_clients.openai_client import AIProviderConfig as OpenAIAIProviderConfig
    ...
    from mcp.cost_estimator import CostEstimator
    from utils.error_handler import APIError, NetworkError, handle_errors, safe_execute
...
class AIClientManager:
    ...
    def get_response(
        ...
        try:
            ...
            self.logger.warning(
                f"AI Manager: {provider} failed (count: {self.failed_providers[provider]}), " f"Error: {str(e)}"
            )

            # Retry logic
            if retry_count < self.max_retries:
                self.logger.info(
                    f"AI Manager: Retrying {provider} request " f"(attempt {retry_count + 1}/{self.max_retries})"
                )
    ...
    def get_smart_responses(
        self, user_message: str, system_prompt: Optional[str] = None, task_id: Optional[str] = None
    ) -> Dict[str, str]:
        ...
        participants = self.coordinator.get_participating_providers(user_message, available_providers, [])
        ...
        for provider in participants:
            try:
                ...
                response = self.get_response(
                    provider, user_message, group_prompt, task_id=task_id, agent_type="coordinator"
                )
    ...
    def get_available_providers(self) -> List[str]:
        ...
        return [p for p in self.clients.keys() if self.failed_providers.get(p, 0) < self.max_retries]
    ...
    async def stream_responses(
        ...
        for i, provider in enumerate(participants):
            yield {"type": "provider_starting", "provider": provider, "position": i + 1, "total": len(participants)}
            ...
            try:
                ...
                if response and not response.startswith("Error:"):
                    ...
                    yield {"type": "provider_completed", "provider": provider, "response": response}

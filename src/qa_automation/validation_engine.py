...
class DataIntegrityChecker:
    ...
    async def check_required_fields(self, rule: ValidationRule, record: Dict[str, Any], record_id: str) -> List[ValidationIssue]:
    ...
    async def check_data_types(self, rule: ValidationRule, record: Dict[str, Any], record_id: str) -> List[ValidationIssue]:
    ...
    async def check_doi_format(self, rule: ValidationRule, record: Dict[str, Any], record_id: str) -> List[ValidationIssue]:
    ...
    async def check_year_range(self, rule: ValidationRule, record: Dict[str, Any], record_id: str) -> List[ValidationIssue]:
    ...
    async def check_author_format(self, rule: ValidationRule, record: Dict[str, Any], record_id: str) -> List[ValidationIssue]:
...
class ConsistencyValidator:
    ...
    async def check_meta_analysis_consistency(self, rule: ValidationRule, data: Dict[str, Any]) -> List[ValidationIssue]:
        ...
        for meta_analysis in meta_analyses:
            ...
            if 'overall_effect' in meta_analysis:
                ...
                if individual_effects:
                    ...
                    if abs(overall_effect - mean_individual) > 2 * statistics.stdev(individual_effects) if len(individual_effects) > 1 else 0:
                        issues.append(ValidationIssue(
                            rule_id=rule.rule_id,
                            severity=ValidationSeverity.MEDIUM,
                            category=rule.category,
    ...
    async def validate_review_data(self, data: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            result = ValidationResult(
                ...
                passed_checks=len([issue for issue in all_issues if issue.severity in [ValidationSeverity.LOW, ValidationSeverity.INFO]]),
                failed_checks=len([issue for issue in all_issues if issue.severity in [ValidationSeverity.CRITICAL, ValidationSeverity.HIGH, ValidationSeverity.MEDIUM]]),

...
class ResearchToolIntegration(ABC):
    ...
    @abstractmethod
    async def execute_analysis(self, analysis_type: AnalysisType, data: Dict[str, Any], 
                             parameters: Dict[str, Any]) -> AnalysisResult:
        """Execute analysis with the external tool"""
        pass
    
    @abstractmethod
    def generate_script(self, analysis_type: AnalysisType, data: Dict[str, Any], 
...
"""
    ...
    def _parse_r_output(self, result: Dict[str, Any], analysis_type: AnalysisType,
                       data: Dict[str, Any], parameters: Dict[str, Any], 
                       script: str) -> AnalysisResult:
        ...
        if os.path.exists(results_file):
            with open(results_file, 'r') as f:
                ...
                if 'summary' in json_results:
                    ...
                    effect_sizes = {
                        'overall': summary.get('overall_effect', 0)
                    }
...
class GradeProIntegration(ResearchToolIntegration):
    ...
    def _create_grade_table(self, assessments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        table_data = {
            ...
            'headers': ['Outcome', 'Quality', 'Risk of Bias', 'Inconsistency', 'Indirectness', 'Imprecision', 'Publication Bias'],
            'rows': []
        }
    ...
    async def test_research_tools():
        ...
        if grade_available:
            ...
            grade_result = await grade_pro.execute_analysis(
                AnalysisType.RISK_OF_BIAS_ASSESSMENT,
                grade_data,
                {}
            )

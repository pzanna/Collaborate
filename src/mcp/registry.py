...
class AgentInstance:
    ...
    def is_available(self) -> bool:
        ...
        return (
            self.is_healthy
            and len(self.current_tasks) < self.registration.max_concurrent
            and self.registration.status == "available"
        )
...
class AgentRegistry:
    ...
    async def get_available_agents(self, capability: str) -> List[str]:
        ...
        async with self._lock:
            ...
            available_agents.sort(key=lambda aid: self.agents[aid].load_factor)

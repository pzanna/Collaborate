...
try:
    ...
    from .fanout_manager import TaskFanoutManager, FanoutStrategy
    from .rm_system_prompt import get_enhanced_rm_prompt, suggest_parallelism, validate_parallelism_value
except ImportError:
    from protocols import ResearchAction, AgentResponse
    from dependency_manager import TaskDependencyManager
    from fanout_manager import TaskFanoutManager, FanoutStrategy
...
class ParallelismCoordinator:
    ...
    def _extract_item_count(self, task: ResearchAction) -> int:
        ...
        if isinstance(payload, dict):
            ...
            item_indicators = [
                'queries', 'documents', 'files', 'items', 'chunks',
                'sources', 'urls', 'papers', 'results', 'data_points'
            ]

...
class CostUsage:
    ...
    tokens_used: int = 0
    cost_usd: float = 0.0
    provider_breakdown: Dict[str, Dict[str, Union[int, float]]] = field(default_factory=dict)
    agent_breakdown: Dict[str, Dict[str, Union[int, float]]] = field(default_factory=dict)
...
class CostEstimator:
    ...
    def __init__(self, config_manager):
        ...
        self.token_costs = {
            "openai": {
                ...
                "gpt - 4.1 - mini": {"input": 0.00015, "output": 0.0006},  # Assumed same as 4o - mini
        ...
        self.logger.info("CostEstimator initialized with thresholds")

    def estimate_task_cost(
        self, query: str, agents: List[str], parallel_execution: bool = False, context_content: Optional[str] = None
    ) -> CostEstimate:
        """
        ...
        estimated_cost = (input_tokens / 1000) * cost_per_1k["input"] + (output_tokens / 1000) * cost_per_1k["output"]
    ...
    def start_cost_tracking(self, task_id: str, session_id: str) -> None:
        ...
        self.active_sessions[task_id] = CostUsage(task_id=task_id, session_id=session_id, start_time=datetime.now())
    ...
    def record_usage(
        ...
        self.logger.info(f"Recorded usage for {task_id}: {total_tokens} tokens, ${total_cost:.4f}")
    ...
    def end_cost_tracking(self, task_id: str) -> Optional[CostUsage]:
        ...
        self.logger.info(
            f"Completed cost tracking for {task_id}: " f"{usage.tokens_used} tokens, ${usage.cost_usd:.4f}"
        )
    ...
    def should_proceed_with_task(self, estimate: CostEstimate, session_id: str) -> tuple[bool, str]:
        ...
        if session_cost + estimate.estimated_cost_usd > self.cost_thresholds["session_limit"]:
            return (
                False,
                f"Session cost limit exceeded: ${session_cost:.2f} + ${estimate.estimated_cost_usd:.2f} > ${self.cost_thresholds['session_limit']}",
            )
        ...
        if today_cost + estimate.estimated_cost_usd > self.cost_thresholds["daily_limit"]:
            return (
                False,
                f"Daily cost limit exceeded: ${today_cost:.2f} + ${estimate.estimated_cost_usd:.2f} > ${self.cost_thresholds['daily_limit']}",
            )

        # Check emergency stop
        if estimate.estimated_cost_usd > self.cost_thresholds["emergency_stop"]:
            return False, f"Task cost exceeds emergency threshold: ${estimate.estimated_cost_usd:.2f}"
    ...
    def get_cost_recommendations(self, estimate: CostEstimate) -> Dict[str, Any]:
        ...
        """
        recommendations = {"current_tier": estimate.task_complexity.value, "suggestions": [], "alternatives": {}}

        # Cost - based recommendations
        if estimate.estimated_cost_usd > 1.0:
            recommendations["suggestions"].append("Consider breaking down into smaller sub - tasks")
        ...
        if estimate.parallel_factor > 1:
            recommendations["suggestions"].append("Sequential execution would reduce cost but increase time")
        ...
        if estimate.task_complexity == CostTier.HIGH:
            recommendations["suggestions"].append("Use memory agent to avoid redundant information gathering")
    ...
    def get_usage_summary(self, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if session_id:
            session_usage = [usage for usage in self.active_sessions.values() if usage.session_id == session_id]
            ...
            return {
                "total_active_sessions": len(set(usage.session_id for usage in all_sessions)),
    ...
    def _assess_task_complexity(self, query: str, agents: List[str], parallel_execution: bool) -> CostTier:
        ...
        medium_complexity_keywords = ["analyze", "compare", "summarize", "explain", "relationship", "trend", "pattern"]
    ...
    def _generate_cost_reasoning(
        self, query: str, agents: List[str], complexity: CostTier, parallel_execution: bool, tokens: int, cost: float
    ) -> str:
        """Generate human - readable cost reasoning."""
        factors = []

        factors.append(f"Query complexity: {complexity.value}")
        factors.append(f"Agent count: {len(agents)} ({', '.join(agents)})")

        if parallel_execution:
            factors.append("Parallel execution increases throughput but token usage")

        factors.append(f"Estimated {tokens:,} tokens (${cost:.4f})")

        return "; ".join(factors)

    def _get_session_cost(self, session_id: str) -> float:
        """Get total cost for a session."""
        return sum(usage.cost_usd for usage in self.active_sessions.values() if usage.session_id == session_id)
    ...
    def _check_cost_thresholds(self, usage: CostUsage) -> None:
        """Check if cost thresholds are exceeded and log warnings."""
        ...
        if session_cost > self.cost_thresholds["session_warning"]:
            self.logger.warning(
                f"Session cost warning: ${session_cost:.2f} exceeds " f"${self.cost_thresholds['session_warning']}"
            )

        # Daily warnings
        if daily_cost > self.cost_thresholds["daily_warning"]:
            self.logger.warning(
                f"Daily cost warning: ${daily_cost:.2f} exceeds " f"${self.cost_thresholds['daily_warning']}"
            )

        # Emergency alerts
        if daily_cost > self.cost_thresholds["emergency_stop"]:
            self.logger.critical(
                f"EMERGENCY: Daily cost ${daily_cost:.2f} exceeds "
                f"emergency threshold ${self.cost_thresholds['emergency_stop']}"
            )

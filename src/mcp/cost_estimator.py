...
class CostEstimator:
    ...
    def estimate_task_cost(self, query: str, agents: List[str], 
        ...
        reasoning = self._generate_cost_reasoning(
            query, agents, complexity, parallel_execution, 
    ...
    def should_proceed_with_task(self, estimate: CostEstimate, session_id: str) -> tuple[bool, str]:
        ...
        if session_cost + estimate.estimated_cost_usd > self.cost_thresholds['session_limit']:
            return False, f"Session cost limit exceeded: ${session_cost:.2f} + ${estimate.estimated_cost_usd:.2f} > ${self.cost_thresholds['session_limit']}"
        
        # Check daily limits
        today_cost = self._get_daily_cost()
        if today_cost + estimate.estimated_cost_usd > self.cost_thresholds['daily_limit']:
            return False, f"Daily cost limit exceeded: ${today_cost:.2f} + ${estimate.estimated_cost_usd:.2f} > ${self.cost_thresholds['daily_limit']}"
    ...
    def get_usage_summary(self, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if session_id:
            session_usage = [usage for usage in self.active_sessions.values() 
    ...
    def _generate_cost_reasoning(self, query: str, agents: List[str], 
                               complexity: CostTier, parallel_execution: bool,
                               tokens: int, cost: float) -> str:
    ...
    def _get_session_cost(self, session_id: str) -> float:
        ...
        return sum(
            usage.cost_usd for usage in self.active_sessions.values()
            if usage.session_id == session_id
        )
    ...
    def _check_cost_thresholds(self, usage: CostUsage) -> None:
        ...
        if session_cost > self.cost_thresholds['session_warning']:
            self.logger.warning(
                f"Session cost warning: ${session_cost:.2f} exceeds "
                f"${self.cost_thresholds['session_warning']}"
            )
        
        # Daily warnings
        if daily_cost > self.cost_thresholds['daily_warning']:
            self.logger.warning(
                f"Daily cost warning: ${daily_cost:.2f} exceeds "
                f"${self.cost_thresholds['daily_warning']}"
            )

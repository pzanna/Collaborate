...
class TaskFanoutManager:
    ...
    async def create_fanout_task(self, 
                                research_action: ResearchAction,
                                parallelism: int,
                                strategy: FanoutStrategy = FanoutStrategy.ROUND_ROBIN,
                                custom_splitter: Optional[Callable] = None,
                                custom_aggregator: Optional[Callable] = None) -> List[ResearchAction]:
    ...
    async def complete_subtask(self, subtask_id: str, result: Dict[str, Any], success: bool = True) -> Optional[Dict[str, Any]]:
        ...
        async with self._lock:
            ...
            if fanout_task.is_complete:
                ...
                self.logger.log_event(
                    LogLevel.INFO, LogEvent.TASK_COMPLETION,
    ...
    async def _default_task_splitter(self, 
                                   research_action: ResearchAction, 
                                   parallelism: int, 
                                   strategy: FanoutStrategy) -> List[ResearchAction]:
    ...
    async def get_all_fanout_tasks(self) -> Dict[str, Dict[str, Any]]:
        ...
        async with self._lock:
            return {
                ...
                for task_id, task in self.fanout_tasks.items()
            }

            ...
            if success:
                ...
                for dependent_id in task_node.dependents:
                    if dependent_id in self.task_graph:
                        ...
                        if (dependent_node.is_ready and 
                            dependent_node.status == TaskStatus.PENDING):
                ...
                self.logger.log_task_completion(
                    ...
                    duration=(task_node.completed_at - task_node.started_at).total_seconds() if task_node.started_at else 0,
                    success=True,
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            return {
                ...
                'average_dependencies': sum(len(node.dependencies) for node in self.task_graph.values()) / max(len(self.task_graph), 1)
            }

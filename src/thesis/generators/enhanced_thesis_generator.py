...
"""
    ...
    def generate_research_questions(self, gaps: List[ResearchGap], framework: ConceptualFramework) -> List[ResearchQuestion]:
...
"""
        ...
        if framework:
            framework_description = framework.get('description', '') if isinstance(framework, dict) else framework.description
            framework_diagram = framework.get('diagram_mermaid', '') if isinstance(framework, dict) else framework.diagram_mermaid
        ...
        try:
            ...
            template_data = {
                ...
                'generation_metadata': {
                    ...
                    'study_count': len(prisma_data.get('data_extraction_tables', {}).get('table_1_study_characteristics', {}).get('data', [])),
...
def main():
    ...
    parser.add_argument('--formats', nargs='+', choices=['markdown', 'latex', 'html'], 
                       help="Output formats")
    ...
    print(f"📊 {len(result['themes'])} themes, {len(result['gaps'])} gaps, {len(result['research_questions'])} research questions")

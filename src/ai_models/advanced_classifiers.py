...
class EnsembleClassificationModel(AdvancedClassificationModel):
    ...
    async def train(
        ...
        self.logger.info(
            f"Training ensemble model {self.model_id} with {len(self.config.get('base_model_configs', []))} base models"
        )
...
class ModelManager:
    ...
    async def compare_models(
        self, model_ids: List[str], test_data: List[Dict[str, Any]]
...
async def demonstrate_advanced_classification():
    ...
    for model_id, performance in comparison_results.items():
        print(
            f"     {model_id}: Accuracy {performance.accuracy:.3f}, Overall Score {performance.get_overall_score():.3f}"
        )

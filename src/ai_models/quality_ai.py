...
class GRADEAssessment:
    ...
    def calculate_final_grade(self) -> str:
        ...
        # Apply upgrades
        final_score += self.large_effect_upgrade + self.dose_response_upgrade + self.confounding_upgrade
...
class RoB2Assessor:
    ...
    async def _assess_domain(self, study_data: Dict[str, Any], domain: str) -> QualityDomain:
        ...
        study_text = (
            f"{study_data.get('title', '')} {study_data.get('abstract', '')} {study_data.get('methods', '')}".lower()
        )
    ...
    def _calculate_numeric_score(self, domain_assessments: List[QualityDomain]) -> float:
        ...
        risk_scores = {BiasRisk.LOW: 1.0, BiasRisk.SOME_CONCERNS: 0.6, BiasRisk.HIGH: 0.2, BiasRisk.UNCLEAR: 0.4}
...
class GRADEAssessor:
    ...
    async def assess_evidence(
        self, studies: List[Dict[str, Any]], outcome: str, synthesis_results: Dict[str, Any]
    ) -> GRADEAssessment:
...
class QualityAI:
    ...
    async def assess_study_quality(self, study_data: Dict[str, Any], assessment_tool: QualityTool) -> QualityAssessment:
    ...
    async def generate_quality_summary(self, assessments: List[QualityAssessment]) -> Dict[str, Any]:
        ...
        common_limitations = [limitation for limitation, count in Counter(all_limitations).most_common(5)]
...
async def demonstrate_quality_ai():
    ...
    config = {"quality_thresholds": {"ai_confidence_threshold": 0.7, "human_review_threshold": 0.6}}
    ...
    print(
        f"   Quality Range: {quality_summary['quality_statistics']['quality_range'][0]:.2f} - {quality_summary['quality_statistics']['quality_range'][1]:.2f}"
    )
    ...
    print(
        f"     Human Review Required: {quality_summary['ai_performance']['human_review_required']} ({quality_summary['ai_performance']['human_review_percentage']:.1f}%)"
    )
    ...
    synthesis_results = {
        "pooled_effect": {"effect_size": 0.45, "confidence_interval": [0.25, 0.65]},
        "heterogeneity": {"i_squared": 0.35},
        "publication_bias": {"egger_test": {"p_value": 0.15}, "begg_test": {"p_value": 0.22}},
    }

    grade_assessment = await quality_ai.assess_evidence_certainty(
        studies, "primary_efficacy_outcome", synthesis_results
    )

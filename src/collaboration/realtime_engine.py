...
class RealtimeCollaborationEngine:
    ...
    async def _send_error(self, user_id: str, message: str):
        ...
        if user_id in self.active_users:
            try:
                await self.active_users[user_id].websocket.send(json.dumps({
                    'type': 'error',
                    'message': message
                }))
            except Exception as e:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                if result:
                    ...
                    cursor.execute("""
                        ...
                        (project_id, screened_studies, included_studies, excluded_studies, uncertain_studies, last_updated)
                        VALUES (?, ?, ?, ?, ?, ?)
                    """, (
                        project_id,
                        screened,
...
async def create_collaboration_client(user_id: str, username: str, project_id: str, 
                                    role: str = "reviewer", server_url: str = "ws://localhost:8765"):
...
async def send_screening_decision(websocket: Any, study_id: str, 
                                decision: str, criteria: Optional[List[str]] = None, 
                                notes: Optional[str] = None, confidence: float = 0.5):

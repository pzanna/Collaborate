...
class CostEstimator:
    ...
    def __init__(self, config_manager: ConfigManager):
        ...
        self.daily_usage: Dict[str, float] = defaultdict(float)
                ...
                # Load cost configurations from config manager
    ...
    def estimate_cost(
        ...
        Args:
            provider: AI provider (e.g., 'openai', 'xai')
            model: Model name
    ...
    def _load_token_costs(self) -> Dict[str, Dict[str, Dict[str, float]]]:
        """Load token costs from config with fallback to defaults"""
        default_costs = {
            "openai": {
        ...
        return self.config_manager.get("cost_settings.token_costs", default_costs)
    
    def _load_cost_thresholds(self) -> Dict[str, float]:
        """Load cost thresholds from config with fallback to defaults"""
        default_thresholds = {
            "session_warning": 1.0,
            "session_limit": 5.0,
            "daily_warning": 10.0,
        ...
        return self.config_manager.get("cost_settings.cost_thresholds", default_thresholds)
    
    def _load_complexity_multipliers(self) -> Dict[CostTier, float]:
        """Load complexity multipliers from config with fallback to defaults"""
        default_multipliers = {
            "LOW": 1.0,
            "MEDIUM": 2.5,
            "HIGH": 5.0,
        ...
        config_multipliers = self.config_manager.get("cost_settings.complexity_multipliers", default_multipliers)
        
        # Convert string keys to CostTier enum
        return {
            CostTier.LOW: config_multipliers.get("LOW", 1.0),
            CostTier.MEDIUM: config_multipliers.get("MEDIUM", 2.5),
            CostTier.HIGH: config_multipliers.get("HIGH", 5.0),
            CostTier.CRITICAL: config_multipliers.get("CRITICAL", 10.0),
        }
